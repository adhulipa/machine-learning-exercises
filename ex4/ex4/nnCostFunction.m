...
a2 = cat(1, ones(1, size(a2,2)), a2); %add bias unit
h = sigmoid(Theta2 * a2);
h = h';
J = (-1/m) * sum(sum(((tempy .* log(h)) + ((1-tempy) .* log(1-h)))));

%regularizing cost
costReg = (lambda / (2 * m)) * ((sum(sum((Theta1(:,2:end) .^ 2))) ...
    + sum(sum(Theta2(:,2:end) .^ 2))));
J = J + costReg;

%implementing back prop algorithm
for t = 1:m
